...
from tensorflow.python.ops import array_ops, math_ops, tensor_array_ops, variable_scope as vs
from tensorflow.python.util import nest

from tensorflow.python.ops.rnn_cell_impl import RNNCell

def _like_rnncell(cell):
    return isinstance(cell, RNNCell)
...
def _concat(prefix, suffix):
    prefix = [prefix] if isinstance(prefix, int) else prefix
    suffix = [suffix] if isinstance(suffix, int) else suffix
    prefix = tf.reshape(tf.cast(prefix, dtypes.int32), [-1])
    suffix = tf.reshape(tf.cast(suffix, dtypes.int32), [-1])
    return array_ops.concat([prefix, suffix], axis=0)
...
def rnn_teacher_force(inputs, cell, sequence_length, initial_state, scope='dynamic-rnn-teacher-force'):
    ...
    def loop_fn(time, cell_output, cell_state, loop_state):
        ...
        return (elements_finished, next_input, next_cell_state, emit_output, None)
